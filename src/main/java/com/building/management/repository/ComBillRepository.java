...
public interface ComBillRepository extends CrudRepository<CompanyBill, String> {
    @Query(value = "SELECT SD.MA_DK,SD.MA_CT,CT.TEN_CT,DV.TEN_DV,IF (:ngayBD < SD.NGAY_BD, SD.NGAY_BD, :ngayBD) AS NGAY_BD, IF (:ngayKT > SD.NGAY_KT, SD.NGAY_KT, :ngayKT) AS NGAY_KT, DV.DON_GIA_CS, (CT.DIEN_TICH*300000) AS MAT_BANG, DATEDIFF(IF (:ngayKT > SD.NGAY_KT, SD.NGAY_KT, :ngayKT), IF (:ngayBD < SD.NGAY_BD, SD.NGAY_BD, :ngayBD)) AS SO_NGAY,\n" +
            "    CASE\n" +
            "    WHEN CT.SO_NV > 10 AND CT.DIEN_TICH > 100 THEN DATEDIFF(IF (:ngayKT > SD.NGAY_KT, SD.NGAY_KT, :ngayKT), IF (:ngayBD < SD.NGAY_BD, SD.NGAY_BD, :ngayBD))*DV.DON_GIA_CS*(1 + (FLOOR((CT.SO_NV - 10)/5) + FLOOR((CT.DIEN_TICH - 100)/10))*0.05)\n" +
            "    WHEN CT.SO_NV > 10 THEN DATEDIFF(IF (:ngayKT > SD.NGAY_KT, SD.NGAY_KT, :ngayKT), IF (:ngayBD < SD.NGAY_BD, SD.NGAY_BD, :ngayBD))*DV.DON_GIA_CS*(1 + (FLOOR((CT.SO_NV - 10)/5))*0.05)\n" +
            "    WHEN CT.DIEN_TICH > 100 THEN DATEDIFF(IF (:ngayKT > SD.NGAY_KT, SD.NGAY_KT, :ngayKT), IF (:ngayBD < SD.NGAY_BD, SD.NGAY_BD, :ngayBD))*DV.DON_GIA_CS*(1 + (FLOOR((CT.DIEN_TICH - 100)/10))*0.05)\n" +
            "    ELSE DATEDIFF(IF (:ngayKT > SD.NGAY_KT, SD.NGAY_KT, :ngayKT), IF (:ngayBD < SD.NGAY_BD, SD.NGAY_BD, :ngayBD))*DV.DON_GIA_CS\n" +
            "    END AS THUC_TRA\n" +
