...
CREATE TABLE NHAN_VIEN_CT(
    ...
    SDT VARCHAR(10),
    PRIMARY KEY (MA_NV),
    FOREIGN KEY (MA_CT) REFERENCES CONG_TY(MA_CT) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE SU_DUNG(
    ...
    PRIMARY KEY (MA_DK),
    FOREIGN KEY (MA_CT) REFERENCES CONG_TY(MA_CT) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (MA_DV) REFERENCES DICH_VU(MA_DV) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE THE_RA_VAO(
    ...
    MA_NV VARCHAR(50) NOT NULL,
    PRIMARY KEY (MA_THE),
    FOREIGN KEY (MA_NV) REFERENCES NHAN_VIEN_CT(MA_NV) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE LAM_VIEC(
    ...
    PRIMARY KEY (MA_LV),
    FOREIGN KEY (MA_DV) REFERENCES DICH_VU(MA_DV) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (MA_NV) REFERENCES NHAN_VIEN_TOA(MA_NV) ON DELETE CASCADE ON UPDATE CASCADE
);
...
AS
SELECT SD.MA_DK, SD.MA_CT, CT.TEN_CT,SD.MA_DV, DV.TEN_DV, SD.NGAY_BD, SD.NGAY_KT, DV.DON_GIA_CS, (CT.DIEN_TICH*300000) AS MAT_BANG,DATEDIFF(SD.NGAY_KT,SD.NGAY_BD) AS SO_NGAY,
CASE
...
##################### Tra ve list thuc tra cua cong ty xac dinh theo ngay BD va ngay KT
# SELECT SD.MA_DK,SD.MA_CT,CT.TEN_CT,DV.TEN_DV,IF ('2020-09-15' < SD.NGAY_BD, SD.NGAY_BD, '2020-09-15') AS NGAY_BD, IF ('2020-09-17' > SD.NGAY_KT, SD.NGAY_KT, '2020-09-17') AS NGAY_KT, DV.DON_GIA_CS, (CT.DIEN_TICH*300000) AS MAT_BANG, DATEDIFF(IF ('2020-09-17' > SD.NGAY_KT, SD.NGAY_KT, '2020-09-17'), IF ('2020-09-15' < SD.NGAY_BD, SD.NGAY_BD, '2020-09-15')) AS SO_NGAY,
#     CASE
#     WHEN CT.SO_NV > 10 AND CT.DIEN_TICH > 100 THEN DV.DON_GIA_CS*(1 + (FLOOR((CT.SO_NV - 10)/5) + FLOOR((CT.DIEN_TICH - 100)/10))*0.05)
#     WHEN CT.SO_NV > 10 THEN DV.DON_GIA_CS*(1 + (FLOOR((CT.SO_NV - 10)/5))*0.05)
#     WHEN CT.DIEN_TICH > 100 THEN DV.DON_GIA_CS*(1 + (FLOOR((CT.DIEN_TICH - 100)/10))*0.05)
...
# WHERE CT.MA_CT = SD.MA_CT
#   AND DV.MA_DV = SD.MA_DV
#   AND SD.NGAY_BD < '2020-09-17'
#   AND '2020-09-15' < SD.NGAY_KT
#   AND SD.MA_CT = 'CT01'
# ORDER BY THUC_TRA
# ;
...
CREATE VIEW LUONG_NV_TOA AS
    SELECT DISTINCT LV.MA_LV,LV.MA_NV , LV.MA_DV,LV.NGAY_BD,LV.NGAY_KT,
                    DATEDIFF(LV.NGAY_KT, LV.NGAY_BD) AS SO_NGAY,
                    (DATEDIFF(LV.NGAY_KT, LV.NGAY_BD)*LV.RATE_LUONG) AS LUONG,
                    (HD.THUC_TRA/HD.DON_GIA_CS) AS PT,
                    (DATEDIFF(LV.NGAY_KT, LV.NGAY_BD)*LV.RATE_LUONG)*(HD.THUC_TRA/HD.DON_GIA_CS) AS LUONG_THUC_TRA
    FROM LAM_VIEC AS LV, HOA_DON_CTY AS HD, NHAN_VIEN_TOA AS NV, DICH_VU AS DV
    WHERE  LV.MA_NV = NV.MA_NV AND DV.MA_DV = LV.MA_DV AND HD.MA_DV = LV.MA_DV
;

############# Thuc hien tren server
############# Tra ve luong cua nhan vien toa nha theo ma nv cu the va theo ngay BD, ngay KT
# SELECT DISTINCT LV.MA_LV,LV.MA_NV , LV.MA_DV,IF ('2020-07-05' < LV.NGAY_BD, LV.NGAY_BD, '2020-07-05') AS NGAY_BD, IF ('2020-07-15' > LV.NGAY_KT, LV.NGAY_KT, '2020-07-15') AS NGAY_KT,
#     DATEDIFF(IF ('2020-07-15' > LV.NGAY_KT, LV.NGAY_KT, '2020-07-15'),IF ('2020-07-05' < LV.NGAY_BD, LV.NGAY_BD, '2020-07-05')) AS SO_NGAY,
#     (DATEDIFF(IF ('2020-07-15' > LV.NGAY_KT, LV.NGAY_KT, '2020-07-15'), IF ('2020-07-05' < LV.NGAY_BD, LV.NGAY_BD, '2020-07-05'))*LV.RATE_LUONG) AS LUONG,
#     (HD.THUC_TRA/HD.DON_GIA_CS) AS PT,
#     (DATEDIFF(IF ('2020-07-15' > LV.NGAY_KT, LV.NGAY_KT, '2020-07-15'), IF ('2020-07-05' < LV.NGAY_BD, LV.NGAY_BD, '2020-07-05'))*LV.RATE_LUONG)*(HD.THUC_TRA/HD.DON_GIA_CS) AS LUONG_THUC_TRA
# FROM LAM_VIEC AS LV, HOA_DON_CTY AS HD, NHAN_VIEN_TOA AS NV, DICH_VU AS DV
# WHERE LV.MA_NV = NV.MA_NV
#   AND DV.MA_DV = LV.MA_DV
#   AND LV.MA_DV = HD.MA_DV
#   AND LV.NGAY_BD < '2020-07-15'
#   AND '2020-07-05' < LV.NGAY_KT
#   AND LV.MA_NV = 'T001'
# ORDER BY LUONG_THUC_TRA
# ;

# DROP VIEW LUONG_NV_TOA;

#VIEW_3: Tra ve ra vao cho client
CREATE VIEW THONG_TIN_RA_VAO AS
    ...
    WHERE RV.MA_THE = TR.MA_THE AND TR.MA_NV = NV.MA_NV #AND DATE(RV.CHECK_TIME) = DATE('2020-09-09 16:34:01') AND NV.MA_NV = 'E0001'
...
INSERT INTO DICH_VU
VALUES
('DV01', 'VE SINH', 'CHUAN', '200000'),
('DV02', 'BAO VE' , 'CHUAN' , '200000'),
('DV03', 'AN UONG', 'EXTRA', '500000'),
('DV04', 'TRONG XE', 'EXTRA', '150000'),
('DV05', 'BAO TRI THIET BI', 'EXTRA', '100000')
;
...
#THEM THE RA VAO
INSERT INTO THE_RA_VAO
VALUES ('TE0001', 'E0001'),
       ('TE0002', 'E0002'),
       ('TE0003', 'E0003')
;

INSERT INTO RA_VAO (MA_THE, VI_TRI, TRANG_THAI, CHECK_TIME)
VALUES ('TE0001', 'GATE A', 'IN', '2020-09-10 8:01:20'),
       ('TE0002', 'GATE A', 'IN', '2020-09-10 8:12:12'),
       ('TE0003', 'GATE B', 'IN', '2020-09-10 8:23:12'),
       ('TE0001', 'GATE C', 'OUT', '2020-09-10 20:01:20'),
       ('TE0002', 'GATE C', 'OUT', '2020-09-10 19:12:12'),
       ('TE0003', 'GATE C', 'OUT', '2020-09-10 17:23:12')
       ;